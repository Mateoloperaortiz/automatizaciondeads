...
def generate_simulated_applications(
    ...
    platforms = ['Meta', 'Google', 'X', 'TikTok', 'LinkedIn', 'Direct']  # Plataformas de origen simuladas
    ...
    if max_possible < num_applications:
        log.warning(f"Se solicitaron {num_applications} aplicaciones, pero solo son posibles {max_possible} combinaciones únicas.")
    ...
    app_id_counter = 1
    attempts = 0
    while len(applications) < num_applications and attempts < max_attempts and consecutive_failures < max_consecutive_failures:
        attempts += 1
        ...
        try:
            ...
            posting_date = datetime.fromisoformat(job_posting_date.replace('Z', '+00:00')) if job_posting_date else datetime.now() - timedelta(days=30)
            closing_date = datetime.fromisoformat(job_closing_date.replace('Z', '+00:00')) if job_closing_date else datetime.now() + timedelta(days=30)
            ...
            log.debug(f"Error al procesar fechas para el trabajo {job_id}, usando fechas aleatorias.")
    ...
    if consecutive_failures >= max_consecutive_failures:
        log.warning(f"Se alcanzó el límite de fallos consecutivos ({max_consecutive_failures}). Posiblemente no hay más combinaciones únicas disponibles.")

    log.info(f"Generación de aplicaciones completada: {len(applications)}/{num_applications} aplicaciones generadas en {attempts} intentos.")
    return applications
