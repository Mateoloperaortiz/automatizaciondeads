...
from ..api.meta.ads import get_ad_manager
from ..api.meta.ad_creatives import get_ad_creative_manager
...
def async_publish_adflux_campaign_to_meta(self, campaign_id: int, simulate: bool = False):
    ...
    try:
        ...
        if not ad_account_id.startswith('act_'):
            ...
            logger.info(f"[Tarea {self.request.id}] Añadido prefijo 'act_' al ID de cuenta: {ad_account_id}")
        ...
        objective = campaign.meta_objective or 'OUTCOME_TRAFFIC'  # OUTCOME_LEADS también sería apropiado para empleo
        ...
        ad_link_url = campaign.landing_page_url or job.application_url or app.config.get('DEFAULT_LANDING_PAGE_URL')
        if not ad_link_url:
        ...
        try:
            ad_message = campaign.primary_text or f"¡Aplica ahora para {job.title} en {job.company_name}!"
        except AttributeError:
            # Fallback para compatibilidad
            ad_message = getattr(campaign, 'ad_text', None) or f"¡Aplica ahora para {job.title} en {job.company_name}!"

        try:
            link_title = campaign.headline or job.title
        except AttributeError:
            # Fallback para compatibilidad
            link_title = getattr(campaign, 'ad_headline', None) or job.title

        try:
            ad_link_description = campaign.link_description or job.short_description or "Aplica ahora"
        except AttributeError:
            # Fallback para compatibilidad
            ad_link_description = getattr(campaign, 'ad_description', None) or job.short_description or "Aplica ahora"
        ...
        try:
            ...
            if campaign.creative_image_filename:
                ...
                image_path = os.path.join(app.root_path, uploads_folder, campaign.creative_image_filename)
            ...
            try:
                if getattr(campaign, 'ad_image_path', None):
                    ...
                    logger.info(f"[Tarea {self.request.id}] Usando imagen proporcionada (legacy): {image_path}")
            except AttributeError:
                pass
        ...
        # 3. Publicar en Meta
        logger.info(f"[Tarea {self.request.id}] Preparando para publicar en Meta con cuenta {ad_account_id}")

        # --- Crear Campaña --- #
        campaign_manager = get_campaign_manager()
        if simulate:
            ext_campaign_id = f"FAKE-CAMPAIGN-{campaign.id}-{int(time.time())}"
            logger.info(f"[SIMULAR] [Tarea {self.request.id}] Se crearía campaña '{campaign_name}' ID: {ext_campaign_id}")
        else:
        ...
        if image_path and not simulate:
            try:
                ...
                if os.path.isfile(image_path):
                    ...
                    success, message, image_data = meta_utils.upload_image(ad_account_id, image_path)
                    if success:
                        image_hash = image_data.get('hash')
                        logger.info(f"[Tarea {self.request.id}] Imagen subida con hash: {image_hash}")
                    else:
                        logger.warning(f"[Tarea {self.request.id}] No se pudo subir la imagen: {message}. Continuando sin imagen.")
                else:
                    logger.warning(f"[Tarea {self.request.id}] El archivo de imagen no existe: {image_path}. Continuando sin imagen.")
            except Exception as e:
                logger.warning(f"[Tarea {self.request.id}] Error al subir imagen: {e}. Continuando sin imagen.")
        ...
        logger.info(f"[Tarea {self.request.id}] Campaña {campaign.id} publicada con éxito (simulada)")

        # Devolver resultados exitosos
        results.update({
            'success': True,
            'message': f"Campaña publicada con éxito (simulada)",
