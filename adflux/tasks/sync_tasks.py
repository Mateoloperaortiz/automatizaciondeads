...
def sync_meta_insights_task(self, campaign_id=None, date_preset='last_30d'):
    ...
    logger.info(f"[Tarea {self.request.id}] Iniciando sincronización de insights de Meta para preset: {date_preset}")
    ...
    try:
        ...
        if campaign_id:
            logger.info(f"[Tarea {self.request.id}] Sincronizando insights para campaña específica: {campaign_id}")
            campaigns = Campaign.query.filter_by(id=campaign_id, platform='meta').all()
        else:
            logger.info(f"[Tarea {self.request.id}] Sincronizando insights para todas las campañas de Meta")
        ...
        for campaign in campaigns:
            try:
                ...
                if not campaign.external_id:
                    logger.warning(f"[Tarea {self.request.id}] Campaña {campaign.id} no tiene ID externo de Meta. Omitiendo.")
                    errors.append(f"Campaña {campaign.id} no tiene ID externo de Meta")
                    continue

                # Obtener insights para la campaña
                logger.info(f"[Tarea {self.request.id}] Obteniendo insights para campaña {campaign.id} (Meta ID: {campaign.external_id})")
                ...
                if not success:
                    logger.error(f"[Tarea {self.request.id}] Error al obtener insights para campaña {campaign.id}: {message}")
                    errors.append(f"Error al obtener insights para campaña {campaign.id}: {message}")
                    continue
                ...
                for insight in insights_data:
                    try:
                        ...
                        logger.error(f"[Tarea {self.request.id}] Error al procesar insight para campaña {campaign.id}, fecha {date_str}: {e}")
                        errors.append(f"Error al procesar insight para campaña {campaign.id}, fecha {date_str}: {str(e)}")
                        continue

                # Confirmar cambios para esta campaña
                db.session.commit()
                campaigns_processed += 1
                logger.info(f"[Tarea {self.request.id}] Procesados {len(insights_data)} insights para campaña {campaign.id}")

            except SQLAlchemyError as e:
                db.session.rollback()
                logger.error(f"[Tarea {self.request.id}] Error de base de datos al procesar campaña {campaign.id}: {e}")
                errors.append(f"Error de base de datos al procesar campaña {campaign.id}: {str(e)}")
                continue
            except Exception as e:
                db.session.rollback()
                logger.error(f"[Tarea {self.request.id}] Error inesperado al procesar campaña {campaign.id}: {e}")
                errors.append(f"Error inesperado al procesar campaña {campaign.id}: {str(e)}")
                continue

        # 3. Preparar resultados
        results["success"] = True
        results["message"] = f"Sincronización completada. Procesadas {campaigns_processed} campañas, creados {insights_created} insights, actualizados {insights_updated} insights."
        ...
        if errors:
            logger.warning(f"[Tarea {self.request.id}] Se encontraron {len(errors)} errores durante la sincronización")

        return results

    except Exception as e:
        db.session.rollback()
        logger.error(f"[Tarea {self.request.id}] Error inesperado durante la sincronización de insights: {e}", exc_info=True)
