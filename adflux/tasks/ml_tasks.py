...
def scheduled_train_and_predict():
    ...
    try:
        ...
        try:
            n_clusters_used = (
                model.n_clusters
            )  # Obtener el número de clústeres del modelo entrenado
            app.logger.info(
                f"Asegurando que existan registros de Segmento para {n_clusters_used} clústeres (IDs 0 a {n_clusters_used - 1})..."
            )

            existing_segment_ids = {
                s.id
                for s in db.session.query(Segment.id)
            ...
            for i in range(n_clusters_used):
                if i not in existing_segment_ids:
                    ...
                    segment_description = (
                        f"Segmento generado automáticamente para clúster K-means {i}"
                    )
                    new_segment = Segment(id=i, name=segment_name, description=segment_description)
                    segments_to_add.append(new_segment)
                    app.logger.info(
                        f"Creando registro de segmento faltante para ID {i} ('{segment_name}')."
                    )
            ...
            if segments_to_add:
                ...
                db.session.commit()
                app.logger.info(
                    f"Se añadieron {len(segments_to_add)} nuevos registros de segmento a la base de datos."
                )
            else:
            ...
            app.logger.error(
                "No se pudo determinar n_clusters desde el objeto del modelo. Omitiendo la creación del registro de segmento."
            )
            # Considerar fallar la tarea si esto es crítico
        except SQLAlchemyError as e:
            db.session.rollback()
            app.logger.error(
                f"Error de base de datos al asegurar registros de segmento: {e}", exc_info=True
            )
            raise  # Volver a lanzar para marcar la tarea como fallida
        except Exception as e:
            db.session.rollback()
            app.logger.error(
                f"Error inesperado al asegurar registros de segmento: {e}", exc_info=True
            )
            raise  # Volver a lanzar para marcar la tarea como fallida
        ...
        try:
            ...
            for candidate in all_candidates:
                try:
                    ...
                    if predicted_segment_float is not None:
                        ...
                        update_needed = (
                            current_segment_id is None
                            or current_segment_id != predicted_segment_int
                        )
                        # logger.debug(...) # Mantener logs detallados por ahora si se desea
                        if update_needed:
                        ...
                        app.logger.warning(
                            f"No se encontró predicción de segmento para el candidato {candidate.candidate_id} en el mapa."
                        )

                except Exception as e:
                    app.logger.error(
                        f"Error procesando segmento para el candidato {candidate.candidate_id}: {e}"
                    )
                    error_count += 1
            ...
            if update_count > 0:
                app.logger.info(
                    f"Intentando confirmar {update_count} actualizaciones de segmento..."
                )
                db.session.commit()
                app.logger.info(
                    f"Segmentos actualizados exitosamente para {update_count} candidatos."
                )
            else:
                app.logger.info(
                    "No fue necesario actualizar ningún segmento de candidato."
                )  # Esto ahora significa que ningún segmento cambió O no se predijo ninguno

            if error_count > 0:
                app.logger.warning(
                    f"Se encontraron errores al procesar segmentos para {error_count} candidatos."
                )

        except KeyError as e:
            app.logger.error(
                f"Error al construir el mapa de segmentos - ¿Falta la columna 'segment'? Índice actual: {df_with_segments.index.name}. Error: {e}"
            )
            db.session.rollback()
        except SQLAlchemyError as e:
            db.session.rollback()
            app.logger.error(
                f"Error de base de datos durante la actualización masiva de segmentos: {e}",
                exc_info=True,
            )
            ...
            db.session.rollback()
            app.logger.error(
                f"Error inesperado durante la actualización de la base de datos de segmentos: {e}",
                exc_info=True,
            )
            raise
...
def run_candidate_segmentation_task(self, candidate_ids=None):
    ...
    try:
        ...
        logger.info(
            f"[Tarea {self.request.id}] Prediciendo segmentos para {len(candidate_df)} candidatos..."
        )
        ...
        try:
            ...
            for candidate in candidates:
                try:
                    ...
                    logger.error(
                        f"[Tarea {self.request.id}] Error procesando segmento para candidato {candidate.candidate_id}: {e}"
                    )
                    error_count += 1
            ...
            if update_count > 0:
                db.session.commit()
                logger.info(
                    f"[Tarea {self.request.id}] Segmentos actualizados para {update_count} candidatos."
                )
            else:
                logger.info(
                    f"[Tarea {self.request.id}] No fue necesario actualizar ningún segmento."
                )

        except Exception as e:
            db.session.rollback()
            logger.error(
                f"[Tarea {self.request.id}] Error durante la actualización de segmentos: {e}",
                exc_info=True,
            )
