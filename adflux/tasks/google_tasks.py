...
def async_publish_adflux_campaign_to_google(self, campaign_id: int, simulate: bool = True):
    ...
    logger.info(
        f"[Tarea {self.request.id}] Iniciando proceso de publicación en Google Ads para Campaña AdFlux ID: {campaign_id} (Simular: {simulate})"
    )
    ...
    try:
        ...
        if not campaign_data_for_api.get("landing_page_url") and job and job.application_url:
            ...
            campaign_data_for_api["landing_page_url"] = app.config.get(
                "DEFAULT_LANDING_PAGE_URL", "https://example.com"
            )

        # Asegurarse de que tenemos un headline
        if (
            not campaign_data_for_api.get("headline")
            and not campaign_data_for_api.get("ad_headline")
            and job
        ):
        ...
        if (
            not campaign_data_for_api.get("link_description")
            and not campaign_data_for_api.get("ad_description")
            and job
        ):
            campaign_data_for_api["link_description"] = (
                job.short_description or f"Aplica ahora para {job.title}"
            )
        ...
        customer_id = (
            ...
            or campaign_data_for_api.get("customer_id")
        )

        # Si no se encuentra un ID de cliente, usar un valor simulado para desarrollo
        if not customer_id and os.environ.get("FLASK_ENV", "development") in [
            "development",
            "testing",
        ]:
            logger.warning(
                f"[Tarea {self.request.id}] No se encontró un ID de cliente de Google Ads. Usando valor simulado para desarrollo."
            )
            customer_id = "1234567890"  # Valor simulado para desarrollo
        elif not customer_id:
            logger.error(
                f"[Tarea {self.request.id}] No se proporcionó un ID de cliente de Google Ads"
            )
        ...
        try:
            ...
            if not campaign_manager or not hasattr(campaign_manager, "publish_campaign"):
                logger.error(
                    f"[Tarea {self.request.id}] No se pudo inicializar el gestor de campañas de Google Ads"
                )
                return {
                    "success": False,
            ...
            if simulate:
                logger.info(
                    f"[Tarea {self.request.id}] Llamando a función SIMULADA de publicación de Google Ads."
                )
                api_result = campaign_manager.publish_campaign(
                    customer_id, campaign.id, campaign_data_for_api
                )
            else:
                # Placeholder para llamada API real
                logger.warning(
                    f"[Tarea {self.request.id}] Publicación real en Google Ads aún no implementada. Ejecutando simulación en su lugar."
                )
                api_result = campaign_manager.publish_campaign(
                    customer_id, campaign.id, campaign_data_for_api
                )
            ...
            logger.error(
                f"[Tarea {self.request.id}] Error al inicializar o usar el cliente de Google Ads: {str(e)}"
            )
            return {
                "success": False,
        ...
        if not api_result.get("success"):
            msg = (
                f"Error al publicar en Google Ads: {api_result.get('message', 'Error desconocido')}"
            )
        ...
        try:
            ...
            db.session.commit()
            logger.info(
                f"[Tarea {self.request.id}] Actualizada Campaña AdFlux {campaign_id} con IDs externos de Google"
            )
        except SQLAlchemyError as e:
            db.session.rollback()
            logger.error(
                f"[Tarea {self.request.id}] Error al actualizar Campaña AdFlux {campaign_id} con IDs externos: {e}"
            )
