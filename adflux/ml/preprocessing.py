...
def load_candidate_data(candidates: Optional[List] = None) -> pd.DataFrame:
    ...
    if candidates:
        ...
        print(
            "Advertencia: No se proporcionaron candidatos y no se puede consultar la base de datos fuera del contexto de la aplicación."
        )
...
def create_preprocessor() -> ColumnTransformer:
    ...
    text_transformer = Pipeline(
        steps=[
            ...
            ),  # Imputar valores faltantes con cadena vacía
            (
                "tfid",
                TfidfVectorizer(max_features=100),
            ),  # Vectorización TF-IDF con máximo 100 características
        ]
    )
