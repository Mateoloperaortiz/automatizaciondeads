...
def analyze_segments_from_db() -> Dict[str, Any]:
    ...
    from ..models import (
        Candidate,
    )  # Importar dentro de la función para evitar dependencia circular a nivel de módulo
    ...
    try:
        ...
        if model is None or preprocessor is None:
            current_app.logger.error(
                "No se pudo cargar el modelo de segmentación para el análisis."
            )
        ...
        for segment_id in sorted(candidate_df["segment"].unique()):
            ...
            if (
                "years_experience" in segment_df.columns
                and not segment_df["years_experience"].isnull().all()
            ):
            ...
            if (
                "desired_salary" in segment_df.columns
                and not segment_df["desired_salary"].isnull().all()
            ):
            ...
            if (
                "primary_skill" in segment_df.columns
                and not segment_df["primary_skill"].isnull().all()
            ):
            ...
            if segment_data["top_primary_skills"]:
                ...
                segment_data["primary_skills_chart"] = {"labels": chart_labels, "data": chart_data}
            else:
            ...
            if (
                ...
                exp_binned = pd.cut(
                    exp_data, bins=bins, labels=labels, right=False
                )  # right=False incluye 0 en el primer rango
                # Contar ocurrencias en cada rango
                exp_counts = exp_binned.value_counts().sort_index()
                segment_data["experience_chart"] = {
                    "labels": list(
                        exp_counts.index.astype(str)
                    ),  # Asegurar que las etiquetas sean strings
                    "data": list(
                        exp_counts.values.astype(float)
                    ),  # Asegurar que los datos sean floats
                }
            else:
                segment_data["experience_chart"] = {
                    "labels": [],
            ...
            if segment_data["top_overall_skills"]:
                ...
                segment_data["overall_skills_chart"] = {"labels": chart_labels, "data": chart_data}
            else:
            ...
            if (
                "education_level" in segment_df.columns
                and not segment_df["education_level"].isnull().all()
            ):
                ...
                segment_data["education_chart"] = {"labels": chart_labels, "data": chart_data}
            else:
        ...
        current_app.logger.error(
            f"Error durante la función de análisis de segmentos: {e}", exc_info=True
        )
