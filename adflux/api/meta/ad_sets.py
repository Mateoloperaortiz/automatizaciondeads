...
class AdSetManager:
    ...
    def get_ad_sets(self, campaign_id: str) -> Tuple[bool, str, List[Dict[str, Any]]]:
        ...
        try:
            ...
            logger.info(
                f"Se recuperaron {len(ad_sets_list)} conjuntos de anuncios para la campaña {campaign_id}."
            )
    ...
    def create_ad_set(
        ...
        try:
            ...
            logger.info(
                f"Se creó correctamente el conjunto de anuncios '{name}' con ID: {ad_set_id}"
            )

            return (
                True,
                f"Se creó correctamente el conjunto de anuncios '{name}'",
                {
                    "id": ad_set_id,
                    "name": name,
