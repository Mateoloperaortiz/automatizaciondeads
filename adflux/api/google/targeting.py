...
class TargetingManager:
    ...
    def get_location_criteria(self, customer_id: str, query: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Ejecutar consulta
            response = location_criterion_service.search(
                customer_id=customer_id, query=search_query
            )
            ...
            logger.error(
                f"Error inesperado al buscar ubicaciones para la consulta '{query}': {e}", e
            )
            return {"success": False, "message": f"Error inesperado: {str(e)}", "locations": []}
        ...
        try:
            ...
            if location_ids:
                for location_id in location_ids:
                    campaign_criterion_operation = google_client.get_type(
            ...
            if gender:
                ...
                campaign_criterion = campaign_criterion_operation.create
                campaign_criterion.campaign = campaign_resource_name
                campaign_criterion.gender.type_ = getattr(
                    google_client.enums.GenderTypeEnum, gender
                )
                operations.append(campaign_criterion_operation)
            ...
            if age_ranges:
                for age_range in age_ranges:
                    ...
                    campaign_criterion = campaign_criterion_operation.create
                    campaign_criterion.campaign = campaign_resource_name
                    campaign_criterion.age_range.type_ = getattr(
                        google_client.enums.AgeRangeTypeEnum, age_range
                    )
                    operations.append(campaign_criterion_operation)
            ...
            for result in campaign_criterion_response.results:
                criterion_id = campaign_criterion_service.parse_campaign_criterion_path(
                    result.resource_name
                )["criterion_id"]
                criteria_ids.append(criterion_id)

            logger.info(
                f"Se añadieron correctamente {len(criteria_ids)} criterios de targeting a la campaña {campaign_id}."
            )
            ...
            logger.error(
                f"Error inesperado al añadir criterios de targeting a la campaña {campaign_id}: {e}",
                e,
            )
            return {"success": False, "message": f"Error inesperado: {str(e)}", "criteria_ids": []}
    ...
    def get_campaign_targeting_criteria(self, customer_id: str, campaign_id: str) -> Dict[str, Any]:
        ...
        if not google_client:
            return {
                "success": False,
                "message": "No se pudo inicializar el cliente de Google Ads",
                "criteria": {"locations": [], "languages": [], "genders": [], "age_ranges": []},
            }
        ...
        try:
            ...
            for row in response:
                ...
                if criterion_type == "LOCATION":
                    ...
                    locations.append({"id": location_id, "resource_name": location_resource_name})
            ...
            logger.error(
                f"Error inesperado al obtener criterios de targeting para la campaña {campaign_id}: {e}",
                e,
            )
            return {
                "success": False,
