...
class AdGroupManager:
    ...
    def get_ad_groups(self, customer_id: str, campaign_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Se recuperaron {len(ad_groups)} grupos de anuncios para el cliente {customer_id}."
            )
            return {
                "success": True,
            ...
            logger.error(
                f"Error inesperado al obtener grupos de anuncios para el cliente {customer_id}: {e}",
                e,
            )
            return {"success": False, "message": f"Error inesperado: {str(e)}", "ad_groups": []}

    @handle_google_ads_api_error
    def create_ad_group(
        self, customer_id: str, campaign_id: str, name: str, status: str = "PAUSED"
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.info(
                f"Se creó correctamente el grupo de anuncios '{name}' con ID: {ad_group_id}"
            )
            ...
            return {"success": False, "message": f"Error inesperado: {str(e)}", "ad_group_id": None}
