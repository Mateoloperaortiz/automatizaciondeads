...
def generate_campaign_report(start_date, end_date):
    ...
    try:
        ...
        insights = MetaInsight.query.filter(
            MetaInsight.date >= start_date, MetaInsight.date <= end_date
        ).all()
        ...
        for campaign in campaigns:
            ...
            campaign_data = {
                ...
                "start_date": (
                    campaign.start_date.strftime("%Y-%m-%d") if campaign.start_date else "N/A"
                ),
        ...
        report_data["summary"] = {
            ...
            "avg_cpm": round(
                (total_spend / total_impressions * 1000) if total_impressions > 0 else 0, 2
            ),
        }
        ...
        top_campaigns = sorted(report_data["campaigns"], key=lambda x: x["clicks"], reverse=True)[
            :10
        ]
...
def generate_job_report(start_date, end_date):
    ...
    try:
        ...
        top_jobs = sorted(report_data["jobs"], key=lambda x: x["candidate_count"], reverse=True)[
            :10
        ]
...
def generate_candidate_report(start_date, end_date):
    ...
    try:
        ...
        report_data["candidates"].sort(
            key=lambda x: x["years_experience"] if x["years_experience"] is not None else 0,
            reverse=True,
        )
        ...
        for candidate in report_data["candidates"]:
            segment = (
                f"Segmento {candidate['segment_id']}"
