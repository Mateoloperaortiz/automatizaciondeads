...
def list_applications():
    ...
    try:
        ...
        if status and status not in valid_statuses:
            notificar_error_web(
                f"Estado '{status}' no válido. Estados válidos: {', '.join(valid_statuses)}. "
                f"Mostrando todas las aplicaciones.",
                "warning",
            )
            status = None
        ...
        if not isinstance(e, AdFluxError):
            raise AdFluxError(
                mensaje=f"Error inesperado al listar aplicaciones: {str(e)}", codigo=500
            )
        raise
