...
def save_uploaded_image(file_storage):
    ...
    if file_storage and allowed_file(file_storage.filename):
        ...
        )  # Obtener de config con fallback
...
def list_campaigns():
    ...
    for campaign in campaigns:
        ...
        campaign.display_status = CAMPAIGN_STATUS.get(
            campaign.status.upper(), campaign.status.title()
        )
...
def create_campaign():
    ...
    form.job_opening.choices = [
        (jo.job_id, jo.title) for jo in JobOpening.query.order_by(JobOpening.title).all()
    ]
    form.target_segment_ids.choices = [
        (s.id, s.name) for s in Segment.query.order_by(Segment.name).all()
    ]
        ...
        try:
            ...
            if form.creative_image.data:
                ...
                if not saved_filename:
                    flash(
                        "La subida de la imagen falló o el tipo de archivo no está permitido.",
                        "warning",
                    )
                    # Decidir si continuar sin imagen o detenerse

            daily_budget_cents = (
                int(form.daily_budget.data * 100) if form.daily_budget.data else None
            )
    ...
    if form.validate_on_submit():
        try:
            ...
            current_app.logger.error(
                f"Error al actualizar campaña {campaign_id}: {e}", exc_info=True
            )
    ...
    if campaign.external_campaign_id:
        try:
            ...
            if campaign.platform == "meta":
                ...
                ad_sets = MetaAdSet.query.filter_by(
                ...
                for ad_set in ad_sets:
                    ...
                    ad_set_insights = (
                        insights_base_query.filter(MetaInsight.meta_ad_set_id == ad_set.ad_set_id)
                        .with_entities(
                            func.sum(MetaInsight.spend),
                            func.sum(MetaInsight.impressions),
                            func.sum(MetaInsight.clicks),
            ...
            current_app.logger.error(
                f"Error al obtener insights para la campaña {campaign_id}: {e}", exc_info=True
            )
