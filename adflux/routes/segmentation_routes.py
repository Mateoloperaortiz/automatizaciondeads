    ...
    try:
        ...
        if "error" in analysis_results:
            ...
            summary_stats = {
                ...
                "segmented_candidates": sum(
                    segment_data.get("size", 0) for segment_data in segments_data.values()
                ),
            ...
            for segment_obj in all_segments:
                ...
                try:
                    campaigns = Campaign.query.filter(
                        Campaign.target_segment_ids.contains([int(segment_id)])
                    ).all()
                    associated_campaigns = [
                        {
                            "id": c.id,
                    ...
                    current_app.logger.error(
                        f"Error al obtener campañas para segmento {segment_id}: {e}", exc_info=True
                    )
                ...
                segment_summary.append(
                    {
                        ...
                        "is_active": bool(
                            segment_data
                        ),  # Marcar si el segmento tiene candidatos asignados
                    }
                )
            ...
            try:
                ...
                current_app.logger.error(
                    f"Error al contar candidatos sin segmento: {e}", exc_info=True
                )

            # Ya hemos establecido las estadísticas de resumen anteriormente

            chart_data = (
                {"labels": chart_labels, "data": chart_data_values} if chart_labels else None
            )
...
def run_segmentation_task():
    ...
    try:
        ...
        run_candidate_segmentation_task.delay()
        flash("Tarea de segmentación iniciada. Los resultados se actualizarán en breve.", "success")
    except Exception as e:
...
def edit_segment(segment_id):
    ...
    if form.validate_on_submit():
        try:
            ...
            current_app.logger.error(
                f"Error al actualizar el segmento {segment_id}: {e}", exc_info=True
            )
...
def trigger_segmentation():
    ...
    try:
        ...
        flash(
            f"Tarea de segmentación de candidatos iniciada (ID de Tarea: {task.id}). Los resultados se actualizarán en breve.",
            "info",
        )
    except Exception as e:
