...
from ..models import db, JobOpening, Candidate, Campaign, MetaInsight, Segment
...
def index():
    ...
    except ValueError: start_date_dt = default_start_date_dt; start_date_str = start_date_dt.isoformat()
    try: end_date_dt = datetime.strptime(end_date_str, '%Y-%m-%d').date()
    except ValueError: end_date_dt = default_end_date_dt; end_date_str = end_date_dt.isoformat()
    ...
    try:
        ...
        if segment_counts:
            ...
            seg_labels = [SEGMENT_MAP.get(s, DEFAULT_SEGMENT_NAME) if s is not None else 'Sin segmentar' for s, count in segment_counts]
        ...
        top_campaigns = insights_base_query.with_entities(
        ...
        ).group_by(MetaInsight.meta_campaign_id).order_by(func.sum(MetaInsight.clicks).desc()).limit(5).all()
