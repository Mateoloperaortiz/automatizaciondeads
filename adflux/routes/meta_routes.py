...
from ..models import db, MetaCampaign, MetaAdSet, MetaAd, MetaInsight
from ..schemas import (meta_campaign_schema, meta_campaigns_schema,
...
campaign_model = meta_ns.model('MetaCampaign', {
...
ad_set_model = meta_ns.model('MetaAdSet', {
...
ad_model = meta_ns.model('MetaAd', {
    ...
    'creative_details': fields.Raw(description='Detalles del Creativo (JSON)'), # Usar Raw para dict/json
    'ad_set_id': fields.String(description='ID del Conjunto de Anuncios Padre'),
})

insight_model = meta_ns.model('MetaInsight', {
...
list_parser.add_argument('per_page', type=int, location='args', default=20, help='Elementos por página')
list_parser.add_argument('account_id', type=str, location='args', help='Filtrar por ID de Cuenta Publicitaria')
list_parser.add_argument('status', type=str, location='args', help='Filtrar por estado (ej., ACTIVE)')
...
class CampaignList(Resource):
    ...
    def get(self):
        ...
        return meta_campaigns_schema.dump(pagination.items), 200, {'X-Total-Count': pagination.total}
...
class AdList(Resource):
    ...
    def get(self):
        ...
        if args.get('account_id'): # Filtrar por cuenta a través de la relación campaña/conjunto de anuncios
             query = query.join(MetaAdSet).join(MetaCampaign).filter(MetaCampaign.account_id == args['account_id'])
        if args.get('status'):
...
insight_parser.add_argument('per_page', type=int, location='args', default=30, help='Elementos por página')
insight_parser.add_argument('account_id', type=str, location='args', help='Filtrar por ID de Cuenta Publicitaria')
insight_parser.add_argument('campaign_id', type=str, location='args', help='Filtrar por ID de Campaña')
insight_parser.add_argument('ad_set_id', type=str, location='args', help='Filtrar por ID de Conjunto de Anuncios')
insight_parser.add_argument('ad_id', type=str, location='args', help='Filtrar por ID de Anuncio')
insight_parser.add_argument('start_date', type=str, location='args', help='Fecha de inicio (YYYY-MM-DD)')
...
class InsightList(Resource):
    ...
    def get(self):
        ...
        if args.get('ad_id'):
            ...
            query = query.filter_by(ad_set_id=args['ad_set_id']).filter(MetaInsight.ad_id == None)
        elif args.get('campaign_id'):
             query = query.filter_by(campaign_id=args['campaign_id']).filter(MetaInsight.ad_set_id == None).filter(MetaInsight.ad_id == None)
             ...
             query = query.filter(MetaInsight.ad_set_id == None).filter(MetaInsight.ad_id == None) # Los insights a nivel de cuenta típicamente tienen ids de campaña/conjunto/anuncio nulos
