...
def settings():
    ...
    if request.method == 'POST' and 'platform' in request.form and request.form['platform'] == 'meta':
        ...
        if action == 'test_meta' or request.form.get('test_connection') == '1':
            ...
            if app_id and app_secret and access_token and ad_account_id:
                ...
                flash("Se requieren todos los campos para probar la conexión con Meta API.", 'error')
        ...
        # Procesar el formulario completo para guardar configuración
            ...
            try:
                ...
                current_app.logger.error(f"Error al guardar configuración Meta API: {e}", exc_info=True)
    ...
    if request.method == 'POST' and 'platform' in request.form and request.form['platform'] == 'google':
        action = request.form.get('action', '')
            ...
            if client_id and client_secret and developer_token and refresh_token and customer_id:
                ...
                try:
                    ...
                    if result.get('success'):
                        flash(f"Conexión a Google Ads API exitosa: {result.get('message')}", 'success')
                    else:
                        flash(f"Error al conectar a Google Ads API: {result.get('message')}", 'error')
                except Exception as e:
                ...
                flash("Se requieren todos los campos para probar la conexión con Google Ads API.", 'error')
        ...
        # Procesar el formulario completo para guardar configuración
            ...
            try:
                ...
                current_app.logger.error(f"Error al guardar configuración Google Ads API: {e}", exc_info=True)
        ...
        if request.method == 'POST' and 'platform' in request.form and request.form['platform'] == 'google' and request.form.get('generate_config') == '1':
            client_id = request.form.get('client_id')
            client_secret = request.form.get('client_secret')
            developer_token = request.form.get('developer_token')
            refresh_token = request.form.get('refresh_token')
            ...
            if client_id and client_secret and developer_token and refresh_token:
                try:
                    ...
                    if success:
                        flash(f"Archivo de configuración de Google Ads generado: {message}", 'success')
                    else:
                        flash(f"Error al generar archivo de configuración: {message}", 'error')
                except Exception as e:
                    flash(f"Error al generar archivo de configuración de Google Ads: {str(e)}", 'error')
                    current_app.logger.error(f"Error al generar archivo de configuración de Google Ads: {e}", exc_info=True)
            else:
                flash("Se requieren todos los campos para generar el archivo de configuración de Google Ads.", 'error')

    # Procesar formulario de Gemini API
    if request.method == 'POST' and 'platform' in request.form and request.form['platform'] == 'gemini':
        action = request.form.get('action', '')
            ...
            if api_key and api_key != 'None' and len(api_key) >= 5:
                ...
                try:
                    ...
                    if success:
                        ...
                        flash(f"Conexión a Gemini API exitosa: {message} ({len(models)} modelos disponibles)", 'success')
                        ...
                        try:
                            ...
                            if success and model_data:
                                current_app.logger.info(f"Obtenidos {len(model_data)} modelos de Gemini")
                        except Exception as e:
                ...
                flash("Se requiere una clave API válida para probar la conexión (mínimo 5 caracteres).", 'error')
        ...
        # Procesar el formulario completo para guardar configuración
            ...
            try:
                ...
                current_app.logger.error(f"Error al guardar configuración Gemini API: {e}", exc_info=True)
        ...
        if request.method == 'POST' and 'platform' in request.form and request.form['platform'] == 'gemini' and request.form.get('list_models') == '1':
            api_key = request.form.get('api_key')
            if api_key and api_key != 'None' and len(api_key) >= 5:
                try:
                    client = get_gemini_client(api_key)
                    ...
                    current_app.logger.error(f"Error al listar modelos de Gemini API: {e}", exc_info=True)
            else:
                flash("Se requiere una clave API válida para listar modelos (mínimo 5 caracteres).", 'error')
