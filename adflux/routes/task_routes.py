...
task_status_base = tasks_ns.model(
    "TaskStatusBase",
    {
        "task_id": fields.String(required=True, description="El ID de la tarea Celery"),
        "status": fields.String(
            required=True, description="Estado actual de la tarea (ej., PENDING, SUCCESS, FAILURE)"
        ),
        "error": fields.String(
            description="Detalles del error si el estado de la tarea es FAILURE"
        ),
    },
)

# Estructura específica para un resultado exitoso de tarea de publicación de Meta
meta_publish_result_fields = tasks_ns.model(
    "MetaPublishResult",
    {
        ...
        "external_ad_set_id": fields.String(
            description="ID del conjunto de anuncios de Meta creado"
        ),
        "external_ad_id": fields.String(description="ID del anuncio de Meta creado"),
        "external_audience_id": fields.String(
            description="ID de la audiencia personalizada de Meta creada (si aplica)"
        ),
    },
)

# Estructura específica para un resultado exitoso de tarea de publicación de Google Ads
google_publish_external_ids_fields = tasks_ns.model(
...
google_publish_result_fields = tasks_ns.model(
    "GooglePublishResult",
    {
        "success": fields.Boolean(description="Indica si la operación de publicación tuvo éxito"),
        "message": fields.String(description="Mensaje de estado de la tarea"),
        "external_campaign_id": fields.String(
            description="ID de la campaña de Google Ads creada (ID principal)"
        ),
        "external_ids": fields.Nested(
            google_publish_external_ids_fields,
            description="Diccionario que contiene todos los IDs de entidades de Google Ads creadas",
        ),
    },
)

# Combinar estado base con posibles estructuras de resultado
task_status_response = tasks_ns.model(
    "TaskStatusResponse",
    {
        "task_id": fields.String(required=True, description="El ID de la tarea Celery"),
        "status": fields.String(
        ...
        "result": fields.Raw(
            description="Payload del resultado. La estructura depende del tipo de tarea y el estado. Puede ser una cadena simple (para estados PENDING/otros) o un objeto JSON específico (para SUCCESS/FAILURE de tareas de publicación). Ver MetaPublishResult o GooglePublishResult para estructuras de publicación exitosas."
        ),
        "error": fields.String(
            description="Detalles del error si el estado de la tarea es FAILURE"
        ),
...
class TaskStatusResource(Resource):
    ...
    @tasks_ns.marshal_with(
        task_status_response, code=200, description="Estado de la tarea recuperado con éxito"
    )
    ...
    def get(self, task_id):
        ...
        if task_result.state == "PENDING":
            ...
            response_data["result"] = str(
                task_result.result
            )  # Representación básica en cadena del error
            response_data["error"] = (
                task_result.info if isinstance(task_result.info, str) else repr(task_result.info)
            )
