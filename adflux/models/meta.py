...
class MetaCampaign(db.Model):
    ...
    )  # Enlazar de vuelta a la Cuenta Publicitaria (aún no es un modelo aquí)
...
class MetaAdSet(db.Model):
    ...
    )  # A menudo entero, pero se mantiene como string por seguridad
    created_time = db.Column(DateTime, nullable=True)
    start_time = db.Column(DateTime, nullable=True)
...
class MetaAd(db.Model):
    ...
    creative_details = db.Column(
        JSON, nullable=True
    )  # Almacenar información del creativo como JSON
