...
def sync_meta_all_command(date_preset):
    ...
    click.echo(f"Sincronizando datos de Meta Ads para todas las cuentas usando preajuste de fecha: {date_preset}")
    try:
...
@click.option('--campaign-id', type=int, help='ID de campaña específica para sincronizar insights. Si no se proporciona, se sincronizan todas las campañas.')
@click.option('--date-preset', default='last_30d',
              type=click.Choice(['today', 'yesterday', 'this_month', 'last_month',
                               'this_quarter', 'maximum', 'last_3d', 'last_7d',
                               'last_14d', 'last_28d', 'last_30d', 'last_90d',
...
def sync_insights_command(campaign_id, date_preset):
    ...
    if campaign_id:
        click.echo(f"Sincronizando insights para la campaña {campaign_id} usando preajuste de fecha: {date_preset}")
    else:
        click.echo(f"Sincronizando insights para todas las campañas usando preajuste de fecha: {date_preset}")
    ...
    try:
        ...
        click.echo(f"Tarea de sincronización de insights iniciada en segundo plano. ID de Tarea: {task.id}")
        click.echo("Revisa los logs del worker de Celery para ver el progreso y estado de finalización.")
    except Exception as e:
