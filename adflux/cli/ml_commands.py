...
@click.option(
    "--candidate-id",
    help="ID de candidato específico para segmentar. Si no se proporciona, se segmentan todos los candidatos.",
)
@with_appcontext
...
def analyze_segments_command():
    ...
    try:
        ...
        for segment_id, segment_data in segments.items():
            click.echo(f"\nSegmento {segment_id}:")
            click.echo(
                f"  Tamaño: {segment_data.get('size', 0)} candidatos ({segment_data.get('percentage', 0)}%)"
            )
